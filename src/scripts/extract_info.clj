...
(defn extract-categories []
  (let [content (tabtree/parse-tab-tree "../taganrog-history-kb/facts/houses/wikimapia_houses.tree")
        categories (reduce
                      ...
                      {}
                      content)
        years-addresses (keys (tabtree/parse-tab-tree "../taganrog-history-kb/facts/houses/years.tree"))
        result (reduce
                  (fn [acc k]
                    (let [v (categories k)]
                      (cond
    ...
    (io/write-to-file "../taganrog-history-kb/_generated/years_from_categories.tree" result)
    true))
